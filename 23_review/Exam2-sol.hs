...
-------------------------------------------------------------
merge :: Ord a => Vec n a -> Vec m a -> Vec (n + m) a
merge (x :> xs) (y :> (ys :: Vec n2 a))
  | x < y     = x :> merge xs (y :> ys)
  | otherwise = case plusSucc @n2 (length xs) of Refl -> y :> merge (x :> xs) ys
merge xs ys = xs ++ ys

runningSum :: Vec n Int -> Vec n Int
runningSum = go 0
  where
    go :: Int -> Vec n Int -> Vec n Int
    go _ Nil = Nil
    go s (x :> xs) = let s' = s + x in
                     s' :> go s' xs

mins :: VecList ns Int -> Vec (Length ns) Int
mins VLNil = Nil
mins (v :>> vs) = min v :> mins vs
  where
    min :: Vec a Int -> Int
    min Nil = 0
    min (x :> Nil) = x
...
wither :: Vec n a -> Vec (Div2 n) a
wither Nil = Nil
wither (_ :> Nil) = Nil
wither (_ :> y :> xs) = y :> wither xs

repeater :: Eq a => Vec n a -> Maybe (Fin n)
repeater Nil = Nothing
repeater (_ :> Nil) = Nothing
repeater (x :> y :> xs)
  | x == y = Just FZero
  | otherwise = fmap FSucc (repeater (y :> xs))
